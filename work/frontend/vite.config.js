...
export default defineConfig({
	...
	server: {
		...
		allowedHosts: true,
		proxy: {
			// 本地开发：前端走 /api，由 Vite 代理到后端
			// - 默认走本机 80（如果你用 docker 的 lb 暴露了 80，并在 lb 上反代 /api）
			// - 如果你本地起了 api（例如在 8080），可设置环境变量：
			//   - Windows CMD: set VITE_DEV_API_TARGET=http://localhost:8080
			//   - PowerShell : $env:VITE_DEV_API_TARGET="http://localhost:8080"
			'/api': {
