          ...
          <div className="flex-1">
                    ...
                    <Link to={`/novel/${novel.slug}`} className="group block h-full bg-white rounded-xl overflow-hidden border border-slate-200 shadow-sm hover:border-accent-400 hover:shadow-lg transition-all">
                      <div className="relative aspect-[2/3] overflow-hidden">
                        ...
                        <div className="absolute top-2 right-2 flex gap-1">
                           {/* 小说是否 Premium 由章节级别控制，这里不展示 */}
                        </div>
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                           <span className="text-white text-sm font-medium flex items-center gap-2"><BookOpen className="w-4 h-4"/> Read Now</span>
                        </div>
                      </div>
                      <div className="p-4">
            ...
            {!loading && novels.length === 0 && (
              <div className="text-center py-20 text-slate-500">
                <p className="text-xl">No novels found matching your criteria.</p>
              </div>
            )}
          </div>
        </div>
